...
export const StockCard: React.FC<StockCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200/80 p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between h-full">
      {/* Header: Symbol & Sector */}
      <div>
        <div className="flex items-start justify-between gap-2 mb-1.5">
          <div>
            <div className="flex items-center gap-2">
              <h4 className="text-lg font-black font-display text-slate-900 tracking-tight">
                {stock.symbol}
              </h4>
        ...
        <div className="my-3">
          ...
          <div className="flex items-center gap-2 text-[10px] text-slate-400 font-mono mt-1">
            <span className="font-bold text-slate-500">H: {formatCurrency(stock.high_price)}</span>
            <span>·</span>
            <span className="font-bold text-slate-500">L: {formatCurrency(stock.low_price)}</span>
            {ownedQuantity > 0 && (
              <>
                <span>·</span>
      ...
      <div className="grid grid-cols-2 gap-2 pt-3 border-t border-slate-100 mt-2">
        <button
          onClick={() => onBuy(stock)}
          disabled={!marketOpen}
          className="w-full py-2.5 rounded-xl text-xs font-extrabold bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 transition-colors disabled:opacity-50"
        >
          BUY
        </button>
        <button
          ...
          className="w-full py-2.5 rounded-xl text-xs font-extrabold bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 transition-colors disabled:opacity-50"
        >
          SELL
        </button>
      </div>
